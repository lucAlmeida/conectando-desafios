...
const User = require('../src/usuario/Usuario');
...
describe('Authentication', () => {
  ...
  it('returns user id, username, perfilCorporativo and token when login success', async () => {
    const user = await addUser();
    const response = await postAuthentication({ email: 'user1@mail.com', password: 'P4ssword' });
    expect(response.body.id).toBe(user.id);
    expect(response.body.username).toBe(user.username);
    expect(Object.keys(response.body)).toEqual(['id', 'username', 'perfilCorporativo', 'token']);
  });
  ...
  it.each`
    language   | message
    ${'pt-BR'} | ${ptBR.authentication_failure}
  ...
  it.each`
    language   | message
    ${'pt-BR'} | ${ptBR.inactive_authentication_failure}
...
describe('Token Expiration', () => {
  const putUser = async (id = 5, body = null, options = {}) => {
    ...
    agent = request(app).put('/api/1.0/usuarios/' + id);
  ...
  it('refreshes lastUsedAt when unexpired token is used for unauthenticated endpoint', async () => {
    ...
    const rightBeforeSendingRequest = new Date();
    await request(app).get('/api/1.0/usuarios/5').set('Authorization', `Bearer ${token}`);
    const tokenInDB = await Token.findOne({ where: { token: token } });
    expect(tokenInDB.lastUsedAt.getTime()).toBeGreaterThan(rightBeforeSendingRequest.getTime());
  });
});
